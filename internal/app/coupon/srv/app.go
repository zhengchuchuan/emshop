...
import (
	...
	"emshop/pkg/log"
	sentinelpkg "emshop/pkg/sentinel"
...
type CouponApp struct {
	...
	statsSyncer    *tasks.StatsMergeSyncer
}

var (
	sentinelInitOnce sync.Once
	sentinelRuleOnce sync.Once
	sentinelInitErr  error
	sentinelRuleErr  error
)
...
func NewCouponApp(cfg *config.Config) (*CouponApp, error) {
	...
	if err := configureSentinelLimit(cfg.Server); err != nil {
		return nil, err
	}

	// 创建gRPC服务器，并接入 Sentinel 限流拦截器
	rpcAddr := fmt.Sprintf("%s:%d", cfg.Server.Host, cfg.Server.Port)
	fallbackHandler := sentinelpkg.NewBusinessFallbackHandler(cfg.Server.Name, true)
	sentinelCfg := sentinelpkg.DefaultServerInterceptorConfig(cfg.Server.Name)
	sentinelCfg.IncludeServiceName = true // 统一以 gRPC service 维度做限流
	sentinelCfg.IncludeMethodName = false // 聚合到服务级别，便于统一控制总QPS
	sentinelCfg.FallbackFunc = fallbackHandler.Handle
	sentinelInterceptor := sentinelpkg.NewUnaryServerInterceptor(sentinelCfg)

	rpcSrv := rpcserver.NewServer(
		rpcserver.WithAddress(rpcAddr),
		rpcserver.WithMetrics(cfg.Server.EnableMetrics),
		rpcserver.WithUnaryInterceptor(sentinelInterceptor),
	)
	...
	if service.AsyncFlashSaleEnabled() && !service.UsingTxnFlashSale() {
		// 仅在普通异步方案下启用补偿链路，事务方案已由 RocketMQ 保证最终一致
		batch := 100
		...
		if cfg.Business != nil && cfg.Business.FlashSale != nil {
			if cfg.Business.FlashSale.ReconcileInterval > 0 {
				interval = cfg.Business.FlashSale.ReconcileInterval
			}
			if cfg.Business.FlashSale.ReconcileThreshold >= 0 {
				threshold = cfg.Business.FlashSale.ReconcileThreshold
			}
			if cfg.Business.FlashSale.CompensationMaxPerRun > 0 {
				maxPerRun = cfg.Business.FlashSale.CompensationMaxPerRun
			}
			if cfg.Business.FlashSale.CompensationCooldown > 0 {
				cooldown = cfg.Business.FlashSale.CompensationCooldown
			}
		}
		...
		// 统计合并任务仅保存实例，延后到 Run 中统一启动，便于 Stop 时收尾
		app.statsSyncer = tasks.NewStatsMergeSyncer(dataFactory, redisClient, 1*time.Second)
	}
...
func (app *CouponApp) Run(ctx context.Context) error {
	...
	// 启动后台库存一致性任务，保证 Redis 与 MySQL 数据闭环
	...
	if app.statsSyncer != nil {
		app.statsSyncer.Start()
		log.Info("库存统计合并任务已启动")
	}
	...
	log.Infof("优惠券服务启动成功，gRPC地址: %s", app.rpcServer.Address())
	return nil
}
...
func (app *CouponApp) Stop() error {
	...
	// 停止后台任务
	if app.stockLogSyncer != nil {
		app.stockLogSyncer.Stop()
	}
	if app.reconciler != nil {
		app.reconciler.Stop()
	}
	if app.statsSyncer != nil {
		app.statsSyncer.Stop()
		log.Info("库存统计合并任务已停止")
	}
...
func (app *CouponApp) autoStartFlashSales() {
	...
	// 压测预热：提前拉起指定活动，避免首波请求击穿数据库
	ids := app.collectAutoStartIDs()
	if len(ids) == 0 {
		return
	}
	for _, id := range ids {
...
// collectAutoStartIDs 汇聚自动预热活动ID，优先环境变量方便临时调试
func (app *CouponApp) collectAutoStartIDs() []int64 {
	var ids []int64
	...
	if v := os.Getenv("COUPON_AUTO_START_IDS"); v != "" {
		for _, raw := range strings.Split(v, ",") {
			...
			if err != nil {
				log.Warnf("忽略非法COUPON_AUTO_START_IDS配置: %q", raw)
				continue
			}
			if n > 0 {
				ids = append(ids, n)
	...
	if app != nil && app.config != nil && app.config.Business != nil && app.config.Business.FlashSale != nil {
...
func configureSentinelLimit(serverOpts *options.ServerOptions) error {
	if serverOpts == nil {
		return fmt.Errorf("server配置为空")
	}

	sentinelInitOnce.Do(func() {
		sentinelInitErr = sentinelapi.InitDefault()
	})
	if sentinelInitErr != nil {
		return fmt.Errorf("初始化Sentinel失败: %w", sentinelInitErr)
	}

	resource := fmt.Sprintf("%s:%s", serverOpts.Name, couponpb.Coupon_ServiceDesc.ServiceName)
	const qpsThreshold = 4000.0

	sentinelRuleOnce.Do(func() {
		var ok bool
		ok, sentinelRuleErr = flow.LoadRules([]*flow.Rule{
			{
				Resource:               resource,
		...
		if sentinelRuleErr == nil && !ok {
			sentinelRuleErr = fmt.Errorf("Sentinel规则未成功加载")
		}
		if sentinelRuleErr == nil {
			log.Infof("Sentinel限流生效: resource=%s, qps=%.0f", resource, qpsThreshold)
		}
	})
	if sentinelRuleErr != nil {
		return fmt.Errorf("加载Sentinel限流规则失败: %w", sentinelRuleErr)
	}
