...
type AdminOrderFilter struct {
	...
	UserId      int32  `form:"user_id"`  // 用户ID筛选（可选）
	Status      string `form:"status"`   // 订单状态筛选（可选）
	OrderSn     string `form:"order_sn"` // 订单号模糊查询（可选）
	StartDate   string `form:"start_date"` // 开始日期 YYYY-MM-DD（可选）
	EndDate     string `form:"end_date"`   // 结束日期 YYYY-MM-DD（可选）
}
...
type OrderSearchBySnRequest struct {
	OrderSn string `form:"order_sn" binding:"required"` // 订单号
}

// OrderSearchByUserIdRequest 按用户ID查询请求
type OrderSearchByUserIdRequest struct {
	UserId      int32 `form:"user_id" binding:"required,min=1"` // 用户ID
	Pages       int32 `form:"p"`                                // 页码
	PagePerNums int32 `form:"pnum"`                             // 每页数量
}