...
if [ -n "$str" ]; then
  if [ $JavaVersion -ge 11 ] ; then
    # For G1 - 优化容器环境内存设置
    JAVA_OPTS="-server -Xms512m -Xmx1g -XX:+UseG1GC -XX:MaxGCPauseMillis=250 -XX:+UseGCOverheadLimit -XX:+ExplicitGCInvokesConcurrent $JAVA_OPTS"
  else
	  JAVA_OPTS="-server -Xms512m -Xmx1g -Xmn256m -XX:SurvivorRatio=2 -XX:PermSize=96m -XX:MaxPermSize=256m -XX:MaxTenuringThreshold=15 -XX:+DisableExplicitGC $JAVA_OPTS"
	fi
else
	JAVA_OPTS="-server -Xms256m -Xmx512m -XX:NewSize=128m -XX:MaxNewSize=128m -XX:MaxPermSize=128m $JAVA_OPTS"
fi
